...
class ComputeF1(Callback):
    ...
    def __init__(self, vd):
        self.best_f1 = -1
        self.validation_dataX = vd
        
    def on_epoch_end(self, epoch, logs={}):

        val_pred = np.round(self.model.predict(self.validation_dataX[0]))
        val_f1 = f1_score(self.validation_dataX[1], val_pred, average='samples')
...
def load_data(df):
    ...
    for i in range(len(df)):
        ...
        train_set = list_of_imgs[:160] #160
        val_set = list_of_imgs[160:240] #160-240
        test_set = list_of_imgs[240:400] #240-400
